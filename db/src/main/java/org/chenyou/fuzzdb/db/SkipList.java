...
import org.chenyou.fuzzdb.util.UnsafeHelper;
...
public class SkipList<Key> implements Iterable<Key> {
    ...
    public void insert(final Key key) throws UnsupportedOperationException {
        ...
        Preconditions.checkArgument(x == null || !equal(x.key, key));
        ...
        if (height > getMaxHeight()) {
            ...
            // no need to lock
            // ref to leveldb comments
            this.maxHeight.set(height);
        }
        x = newNode(key, height);
        for (int i = 0; i < height; i++) {
            x.setNextNoFence(i, prev.get(i).nextNoFence(i));
            // TODO test needed, with fence here
    ...
    private static final class Node<Key> {
        ...
        private Node<Key> next(int n) {
            unsafe.loadFence();
            return next.get(n).get();
        }

        private Node<Key> nextNoFence(int n) {
            return next.get(n).get();
        }

        private void setNextNoFence(int n, Node<Key> x) {
            this.next.get(n).set(x);
        }

        private void setNext(int n, Node<Key> x) {
            unsafe.storeFence();
            this.next.get(n).set(x);
        }
